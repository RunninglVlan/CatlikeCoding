...
public static class Functions {
    ...
    static Vector3 Sine(float x, float z, float time) {
        var y = Sine(x, time);
        return new Vector3(x, y, z);
    }

    static Vector3 Sine2D(float x, float z, float time) {
        var y = (Sine(x, time) + Sine(z, time)) * .5f;
        return new Vector3(x, y, z);
    }

    static Vector3 MultiSine(float x, float z, float time) {
        var y = Sine(x, time);
    ...
    static Vector3 MultiSine2D(float x, float z, float time) {
    ...
    static Vector3 Sphere(float u, float v, float time) {
        const float multiplier = 1.3f;
    ...
    static Vector3 Torus(float u, float v, float time) {
    ...
    static float Sine(float angle, float time = 0) => math.sin(math.PI * (angle + time));
    static float Cos(float angle, float time = 0) => math.cos(math.PI * (angle + time));
}
